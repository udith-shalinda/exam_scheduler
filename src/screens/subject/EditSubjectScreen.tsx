...
const AddSubjectScreen = ({ userState, navigation, examState, route }: any) => {
    ...
    React.useEffect(() => {
        if (!userState.token) {
            ...
            onChangeSubject(route?.params);
            const data = examState.exams.find((res: IExam) => res.id === route?.params?.examId)
    ...
    const onUpateSubject = async () => {
        ...
        try {
            ...
            setLoading(false);
            setIsError({isVisible: true, message: 'Updating Subject failed', onOkay: ()=> {setIsError({...error, isVisible: false})}})
        }
    }
